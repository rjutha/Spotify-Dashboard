---
title: "Tyler, the Creator Song Analysis"
format: 
  dashboard:
    logo: logo.jpg
    theme: "theme.scss"
...
//upload data
songs = FileAttachment("data/tyler_album_songs.csv").csv({ typed: true })

//get list of alums
albums = [...new Set(songs.map(d => d.album))].sort()

filtered_albums = [...new Set(filtered_songs.map(d => d.album))].sort()
...
filtered_songs_album = songs.filter(d => selectAlbums.includes(d.album))
```
...
viewof selectAlbums = Inputs.select(albums, {label:null, value:albums, multiple:10})
...
Plot.plot({
  ...
  x: {tickSize:0},
  //customize legend
  color: { scheme: "GnBu", pivot: 0, legend: true, label: "Correlation" },
  marks: [
